...
public class UserController {
    ...
    public ResponseEntity<ApiResponse> registerInfluencer(@Valid @RequestBody RegistrationOtp registrationOtp) {
        try {
            ...
            if (existingInfluencer.isPresent()) {
                ApiResponse apiResponse = new ApiResponse(new java.util.Date(), HttpStatus.BAD_REQUEST.value(), "Account with "+registrationOtp.getEmail()+" already exists", null);
                return ResponseEntity.badRequest().body(apiResponse);
            }
            String otp = emailService.generateCode();
            try {
                // emailService.sendRegistrationOtpOnMail(registrationOtp.getName(), registrationOtp.getEmail(), otp);
                influencerService.saveRegOtpToDB(registrationOtp, otp );
            } catch (Exception e) {
                ApiResponse apiResponse = new ApiResponse(new java.util.Date(), HttpStatus.INTERNAL_SERVER_ERROR.value(), "Error sending mail. Please try again.", null);
                return ResponseEntity.badRequest().body(apiResponse);
            }

            ApiResponse apiResponse = new ApiResponse(new java.util.Date(), HttpStatus.OK.value(), "Registration OTP sent to your email.", otp);
            return ResponseEntity.ok(apiResponse);

        } catch (Exception ex) {
            ex.printStackTrace();
            ApiResponse apiResponse = new ApiResponse(new Date(), HttpStatus.INTERNAL_SERVER_ERROR.value(), "Error occurred while registering influencer.", null);
    ...
    public ResponseEntity<ApiResponse> verifyCode(@RequestBody VerifyRegCodeRequest verifyCodeRequest) {
        ...
        if (existingInfluencer.isPresent()) {
            ApiResponse apiResponse = new ApiResponse(new Date(), HttpStatus.BAD_REQUEST.value(), "Account with "+verifyCodeRequest.getEmail()+" already exists", null);
            return ResponseEntity.badRequest().body(apiResponse);
        }
        ...
        RegistrationOtp latestRegistrationOtp = registrationOtpRepository.findAllByEmail(verifyCodeRequest.getEmail()).stream()
                .sorted(Comparator.comparing(RegistrationOtp::getCreatedAt).reversed())
                .findFirst()
                .orElse(null);

        if (latestRegistrationOtp == null) {
            ApiResponse apiResponse = new ApiResponse(new Date(), 400, "OTP not found. Please try again.", null);
            return ResponseEntity.badRequest().body(apiResponse);
        }
        if (!latestRegistrationOtp.getOtp().equals( verifyCodeRequest.getOtp())) {
            ApiResponse apiResponse = new ApiResponse(new Date(), 400, "Invalid OTP. Please try again.", null);
            return ResponseEntity.badRequest().body(apiResponse);
        }

        influencerService.registerInfluencer(latestRegistrationOtp);
        ApiResponse apiResponse = new ApiResponse(new Date(), HttpStatus.OK.value(), "OTP verified successfully.", null);
        return ResponseEntity.ok(apiResponse);
    }
